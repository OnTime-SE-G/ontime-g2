# services/api-gateway/tests/unit/test_ml_eta_model.py
...
class TestGenerateTrainingData:
    ...
    def test_feature_ranges(self):
        ...
        assert X[:, 0].min() >= 200       # distance >= 200 m
        assert X[:, 1].min() >= 2.0       # speed >= 2 m/s
        assert X[:, 2].min() >= 0         # hour >= 0
        assert X[:, 2].max() <= 23        # hour <= 23
...
class TestPredictEta:
    ...
    def test_reasonable_range(self):
        # 1 km at 10 m/s → physics ≈ 100 s; AI should be in 50-250 s
        result = predict_eta(1000.0, 10.0)
        assert 50 < result.eta_seconds < 250

    def test_rush_hour_slower_than_night(self):
        rush = datetime(2026, 5, 4, 8, 0, tzinfo=timezone.utc)   # Monday 8 am
        night = datetime(2026, 5, 4, 2, 0, tzinfo=timezone.utc)  # Monday 2 am
        result_rush = predict_eta(5000.0, 8.0, dt=rush)
        result_night = predict_eta(5000.0, 8.0, dt=night)
        # Rush hour ETA should be >= night ETA
    ...
    def test_zero_speed_fallback(self):
        ...
        # Speed will be clamped to default — physics path
    ...
    def test_model_used_field_ai_path(self):
        # With normal inputs the AI model should not be clamped
        result = predict_eta(2000.0, 8.0)
        # clamped could be either True or False depending on prediction;
        # simply verify no exception is raised and we get an EtaResult
        assert isinstance(result, EtaResult)
